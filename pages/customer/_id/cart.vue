...
export default {
    ...
    methods: {
        async getCartInfo() {
            this.data = null;
            await axios.get(`http://192.168.191.4:8000/api/product/Order_retrieve/${this.$route.params.id}/`)
        ...
        async sendData() {
            this.loading = true;
            await axios.put(`http://192.168.191.4:8000/api/product/Update_order/${this.data.id}/`, { "is_payed": true, "is_payment_cash": this.is_payment_cash })
                .catch(function (error) {
                if (error.response) {
                    console.log(error.response.data);
                    console.log(error.response.status);
        ...
        async deleteItem(id) {
            this.loading = true;
            await axios.delete(`http://192.168.191.4:8000/api/product/delete_order_product/${id}/`)
                .catch(function (error) {
                if (error.response) {
                    console.log(error.response.data);
                    console.log(error.response.status);
